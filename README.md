...
*Detect. Predict. Explain. Assist.*

**MRI Analysis • Clinical Symptom Modeling • Risk Assessment • Explainable AI • RAG Medical Chatbot**
...
> **This project is developed strictly for educational and research purposes as part of a Final Year B.Tech (AI & ML) capstone project.**
> It is **not** a certified medical device and **must not** be used as a substitute for professional medical diagnosis, advice, or treatment. All predictions are intended to *assist*, not *replace*, qualified healthcare professionals.

---

## 📖 Project Overview

**UteroCare** is an end-to-end, AI-powered **Clinical Decision Support System (CDSS)** built to assist healthcare professionals in the early detection and risk assessment of uterine diseases.
...
The system doesn't just say *"what"* — through **Grad-CAM explainability**, it shows *"where"* and helps clinicians understand *"why"*, which is critical for trust and adoption of AI in real clinical workflows.

### 🎯 Core Capabilities

| Capability | Description |
|---|---|
| 🧠 **MRI Disease Classification** | Deep learning model classifies uterine MRI scans into 4 clinical categories |
| 📋 **Symptom-Based Prediction** | Gradient-boosted model predicts disease likelihood from 15+ clinical symptoms |
| 🔗 **AI Fusion Engine** | Intelligently merges MRI + symptom predictions into one robust diagnosis |
| ⚠️ **Risk Stratification** | Classifies patient risk as Low / Medium / High for triage prioritization |
...
| 🗂️ **Patient Management** | Full patient lifecycle: records, history, predictions, and reports |
...
<details>
<summary><b>🔐 User Authentication & Dashboard</b></summary>
<br/>

- Secure **Login** and **Registration** system for clinicians
- Role-based **Dashboard** summarizing patient statistics, recent predictions, and risk distribution
...
<details>
<summary><b>🔗 AI Fusion Engine — The Core Innovation</b></summary>
<br/>

The **Fusion Engine** is the architectural heart of UteroCare. Relying on a single modality (only imaging, or only symptoms) leaves diagnostic blind spots — MRI can miss early biochemical/functional signs, while symptoms alone are non-specific.

The Fusion Engine intelligently combines:
...
- If MRI confidence is **high** and symptom prediction **agrees** → Fusion reinforces the diagnosis with boosted confidence
- If MRI confidence is **low/ambiguous** → Fusion leans on the **symptom model** to disambiguate
...
This weighted, condition-aware fusion mimics how a real clinician cross-verifies imaging against a patient's reported history, rather than trusting a single data source blindly.

</details>

<details>
<summary><b>⚠️ Disease Risk Prediction</b></summary>
<br/>

A dedicated ML classifier evaluates purely clinical/demographic features to estimate the **overall severity/risk profile** of a patient — independent of imaging.
...
Healthcare AI cannot be a "black box." A model that says *"Fibroid, 91% confidence"* without justification is clinically unusable and legally risky.
...
- ✅ Builds **clinical trust** — doctors can visually verify the AI is "looking" at the correct anatomical region
- ✅ Enables **error detection** — if the model highlights irrelevant regions, clinicians know to disregard the prediction
- ✅ Supports **regulatory & ethical compliance** — explainability is increasingly required for medical AI systems
...
4. Retrieved context + query are passed to the **LLM (Ollama)** for grounded generation
...
flowchart TD
    ...
    B --> C[🧠 MRI Model<br/>EfficientNetV2]
    B --> D[📋 Symptom Model<br/>XGBoost]
    C --> E[🔗 Fusion Engine]
    D --> E
    E --> F[⚠️ Risk Prediction Model]
    F --> G[🔍 Grad-CAM Explainability]
    G --> H[📄 Generate Clinical Report]
    H --> I[🗄️ Store Prediction in Database]
...
| Attribute | Detail |
|---|---|
| **Purpose** | Combine MRI + Symptom predictions into one robust diagnosis |
| **Method** | Confidence-weighted, condition-aware decision fusion |

### 5️⃣ Grad-CAM Explainability Module
...
5. **Fusion** — The Fusion Engine combines both predictions using confidence-weighted logic to produce a single, more reliable diagnosis.
...
- ☁️ **Cloud Deployment** — Containerize and deploy on AWS / Azure / GCP for scalability
- 🏥 **Hospital System Integration** — Connect with real hospital information systems (HIS)
- 🩻 **DICOM Support** — Native support for standard medical imaging format (DICOM) instead of raw images
- 👨‍⚕️ **Doctor Collaboration Tools** — Multi-clinician review, annotations, and second-opinion workflows
...
### 🎓 A Final Year B.Tech Capstone Project — Artificial Intelligence & Machine Learning

**Made with 🧠 + ❤️ for advancing accessible healthcare AI**
